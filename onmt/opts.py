...
def train_opts(parser):
    ...
    # extra setting
    group = parser.add_argument_group('mask_story_def')
    group.add('--disturb_type', '-disturb_type', required=False, default='Noise', help='')
    group.add('--only_step3', '-only_step3', required=False, action='store_true', help='')
    group.add('--train_type', '-train_type', required=False, default='pretrain',
              help='pretrain | noise | nmt_noise | nmt_finetune | conv_finetune')
    group.add('--gm_iter_count', '-gm_iter_count', required=False, default=8, type=int, help='')

    group = parser.add_argument_group('General')
    group.add('--data', '-data', required=True,
              help='Path prefix to the ".train.pt" and '
                   '".valid.pt" file path from preprocess.py')
    group.add('--wh_sample_num', '-wh_sample_num', required=False, type=int, default=5, help='')
    group.add('--wh_disturb_ratio', '-wh_disturb_ratio', required=False, type=float, default=0.1, help='')
    group.add('--noise_lambda', '-noise_lambda', required=False, type=float, default=1.0, help='')
    group.add('--noise_l2', '-noise_l2', required=False, type=float, default=1.0, help='')
    group.add('--bw_adv_w_clip', '-bw_adv_w_clip', required=False, type=float, default=1.0, help='')
    group.add('--fw_adv_w_clip', '-fw_adv_w_clip', required=False, type=float, default=1.0, help='')
    group.add('--fgsm_alpha', '-fgsm_alpha', required=False, type=float, default=1.0, help='')
    ...
    group.add('--g_train_from', '-g_train_from', default='', type=str,
              help="If training from a checkpoint then this is the "
                   "path to the pretrained model's state_dict.")
    group.add('--m_train_from', '-m_train_from', default='', type=str,
              help="If training from a checkpoint then this is the "
                   "path to the pretrained model's state_dict.")
    group.add('--n_train_from', '-n_train_from', default='', type=str,
              help="If training from a checkpoint then this is the "
                   "path to the pretrained model's state_dict.")
...
def translate_opts(parser):
    ...
    group.add('--n_model_load_path', '-n_model_load_path', type=str)
    group.add('--noise_lambda', '-noise_lambda', type=float, default=1.0)
