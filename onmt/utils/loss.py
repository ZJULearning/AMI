...
def build_loss_compute(model, tgt_field, opt, train=True):
    ...
    if opt.lambda_coverage != 0:
        raise ValueError('no support anymore! (loss)')
...
class NewNMTLossCompute(LossComputeBase):
    """
    Standard NMT Loss Computation.
    """
    def __init__(self, criterion, generator, normalization="sents",
                 lambda_coverage=0.0, lambda_align=0.0):
        super(NewNMTLossCompute, self).__init__(criterion, generator)
        self.lambda_coverage = lambda_coverage
        self.lambda_align = lambda_align
    ...
    def __call__(self,
        ...
        shard_state = self._make_shard_state(batch, output, trunc_range, attns)
        if shard_size == 0:
            loss, stats = self._compute_loss(batch, reward_weights, **shard_state)
            return loss / float(normalization), stats
        batch_stats = onmt.utils.Statistics()
        for shard in shards(shard_state, shard_size):
            loss, stats = self._compute_loss(batch, reward_weights, **shard)
            loss.div(float(normalization)).backward()
            batch_stats.update(stats)
        return None, batch_stats
    ...
    def _compute_loss(self, batch, reward_weights, output, target, std_attn=None,
                      coverage_attn=None, align_head=None, ref_align=None):
        n0, n1 = output.size(0), output.size(1)
        bottled_output = self._bottle(output)
        ...
        # print('gtruth: {}'.format(gtruth.size()))
        # print('loss: {}'.format(loss.size()))
        # print('loss[0][:100]: {}'.format(loss[0][:100]))

        if reward_weights is None:
            # print('no reward loss: {}'.format(loss.size()))
            loss = torch.sum(loss)
        else:
            # print('loss: {}'.format(loss.size()))
            # print('reward_weights: {}'.format(reward_weights.size()))
            loss = loss.contiguous().view(n0, n1).transpose(0, 1)  # [N, len]
            loss = torch.sum(loss, 1)
            loss = loss * reward_weights.unsqueeze(1)
            loss = torch.sum(loss)
        stats = self._stats(loss.clone(), scores, gtruth)
