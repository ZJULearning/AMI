...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    if out_file is None:
        ...
        out_temp_file = codecs.open(opt.output+'.temp', 'w+', 'utf-8')

    load_test_model = onmt.decoders.ensemble.load_test_model \
        if len(opt.models) > 1 else onmt.model_builder.load_test_model
    fields, model, model_opt = load_test_model(opt)
    # FIXME 加载noise模型
    n_model_load_path = opt.n_model_load_path
    n_checkpoint = torch.load(n_model_load_path, map_location=lambda storage, loc: storage)
    ...
    translator.out_temp_file = out_temp_file
...
class Translator(object):
    ...
    def translate(
        ...
        for batch in data_iter:
            ...
            for trans in translations:
                ...
                output, pred_out_dict = trans.log(sent_number, return_with_dict=True)
                import json
                self.out_temp_file.write(json.dumps(pred_out_dict) + '\n')
                self.out_temp_file.flush()
                if self.verbose:
                    if self.logger:
    ...
    def _run_encoder(self, batch):
        ...
        # NOTE: add noise
        h_disturbs = self.n_model(memory_bank, n_sample=1)
        memory_bank = memory_bank + self.noise_lambda * h_disturbs[0]
