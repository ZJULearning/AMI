...
class Translation(object):
    ...
    def log(self, sent_number, return_with_dict=False):
        """
        Log translation.
        """
        msg_dict = {}
        msg = ['\nSENT {}: {}\n'.format(sent_number, self.src_raw)]
        msg_dict['sent_id'] = sent_number
        msg_dict['src'] = self.src_raw
        ...
        msg_dict['best_pred'] = pred_sent
        msg_dict['best_score'] = best_score.item()
        ...
        if self.word_aligns is not None:
            ...
            # msg_dict['attn_align'] = pred_align_sent

        if self.gold_sent is not None:
            tgt_sent = ' '.join(self.gold_sent)
            msg.append('GOLD {}: {}\n'.format(sent_number, tgt_sent))
            msg.append(("GOLD SCORE: {:.4f}\n".format(self.gold_score)))
            msg_dict['gold'] = tgt_sent
            msg_dict['gold_score'] = self.gold_score.item()
        ...
        if len(self.pred_sents) > 1:
            ...
            msg_dict['top_preds'] = top_preds

        if return_with_dict:
            return "".join(msg), msg_dict
        return "".join(msg)
